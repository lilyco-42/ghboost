...
`test` 需要一个 Mihomo（Clash.Meta）内核来跑真实延迟。ghboost **自带内核**，无需用户另装 Clash Verge：

- Release 里每个目标平台都附带 `mihomo-<triple>`（Linux/macOS/Android）或 `mihomo-<triple>.exe`（Windows）。
  把它与 `ghboost` 二进制放在**同一目录**，`ghboost test` 会自动发现并使用它。
- 探测顺序：`--mihomo` 显式指定 → `GHBOOST_MIHOMO` 环境变量 → 同目录的内置内核
  （`mihomo` / `mihomo-<triple>`）→ 系统标准路径（`/usr/local/bin`、`/opt/homebrew/bin`、
  Clash Verge 安装目录等）→ PATH 里的 `mihomo`。
- 内核来源：[MetaCubeX/mihomo](https://github.com/MetaCubeX/mihomo) 官方预编译二进制（v1.19.30，SSE2/ARM64 基线，兼容旧 CPU）。
- 许可证：Mihomo 内核为 **GPL-3.0**，作为**独立可执行文件**随 ghboost（MIT）分发，
  属「mere aggregation」——内核自身的 GPL 许可不受影响，ghboost 仍为 MIT。
